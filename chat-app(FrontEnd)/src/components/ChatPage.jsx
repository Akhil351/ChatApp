    ...
    return (
        <div className="flex flex-col h-screen">
            ...
            <header className="dark:border-gray-700 h-20 fixed w-full dark:bg-gray-900 py-5 shadow flex justify-around items-center">
                ...
                <div>
                    <button onClick={handleLogout} className="dark:bg-red-500 hover:dark:bg-red-700 px-3 py-2 rounded-full">Leave Room</button>
                </div>
            </header>

            {/* Chat Content */}
            <main ref={chatBoxRef} className="py-20 px-4 md:px-10 flex-1 w-full md:w-2/3 dark:bg-slate-600 mx-auto overflow-auto">
                {messages.map((message, index) => (
                    <div key={index} className={`flex ${message.sender === currentUser ? "justify-end" : "justify-start"}`}>
                        <div className={`my-2 ${message.sender === currentUser ? "bg-green-800" : "bg-gray-800"} p-2 max-w-xs rounded`}>
                            <div className="flex flex-row gap-2">
                                <img className="h-10 w-10" src={"https://avatar.iran.liara.run/public/47"} alt="avatar" />
            ...
            <div className="fixed bottom-4 w-full h-16">
                <div className="h-full px-4 md:pr-10 flex gap-4 rounded-full items-center justify-between w-full md:w-1/2 mx-auto dark:bg-gray-900">
                    <input
                        value={input}
                        ...
                        className="h-full w-full dark:border-gray-600 dark:bg-gray-800 px-5 py-2 rounded-full focus:outline-none"
                    />
